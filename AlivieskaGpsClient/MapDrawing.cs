...
namespace AlivieskaGpsClient
{
	...
	public static class MapDrawing
	{
		...
		public static void DrawArrow(Graphics g, PointF position, double angle, Point center, Size size, Color InColor, bool View)
		{
            if (View) {
			    double rad = angle * Math.PI / 180.0;
			    float cos = (float)Math.Cos(rad);
			    ...
			    g.FillPolygon(new SolidBrush(InColor), _arrowPointsTransformed);
